...
from Hyperparameters.Training.ActiveFineTuneLearningLoop import query_entropy, active_learning_loop
from Hyperparameters.Utils.Misc import get_device


class Objective:
    def __init__(self, model_name="distilbert/distilbert-base-uncased", csv_path="data/Sentiment/training.csv",
                 seed=42):
        self.seed = seed
    ...
    def _prepare_dataloaders(self):
        ...
        if self.embedder.is_variable_length:
            ...
            cache_train_path = cache_path + "_train_emb_dataset.pt"
            cache_val_path = cache_path + "_val_emb_dataset.pt"

            if os.path.exists(cache_train_path):
                self.train_loader_frozen = torch.load(cache_train_path, weights_only=False)
            else:
                train_loader_pred = DataLoader(train_dataset, batch_size=8,
                                               collate_fn=collate_fn)
                self.train_embed_dataset = self.embedder.embed_dataset(train_loader_pred)
                os.makedirs("cache", exist_ok=True)
                torch.save(self.train_embed_dataset, cache_train_path)

            if os.path.exists(cache_val_path):
                self.val_loader_frozen = torch.load(cache_val_path, weights_only=False)
            else:
                self.val_embed_dataset = self.embedder.embed_dataset(self.val_loader_unfrozen, val=True)
                os.makedirs("cache", exist_ok=True)
                torch.save(self.val_embed_dataset, cache_val_path)
            ...
            self.train_loader_unfrozen = DataLoader(TensorDataset(X_train_tensor, Y_train_tensor), batch_size=16,
                                                    shuffle=True)
    ...
    def __call__(self, trial):
        with mlflow.start_run():
            ...
            return active_learning_loop(
                model,
                get_device(),
                self.train_embed_dataset,
                self.val_embed_dataset,
